...
function renderPokemon(pokemon) {
  ...
  $createdColumn.setAttribute('class', 'column-third');

  const $createdCard = document.createElement('div');
  $createdCard.setAttribute('class', 'pokemon-card');
  $createdColumn.appendChild($createdCard);
  ...
  $createdCard.appendChild($cardText);

  const $h2 = document.createElement('h2');
  $h2.textContent = pokemon.name;
  $createdCard.appendChild($h2);
...
for (let i = 0; i < pokedex.length; i++) {
  const output = renderPokemon(pokedex[i]);
